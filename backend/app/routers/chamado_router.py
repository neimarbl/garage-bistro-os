...
async def abrir_chamado(payload: NovoChamadoRequest, db: Session = Depends(get_db)):
    """
    ...
    chamado = repo.criar_chamado(mesa_id=payload.mesa_id, tipo=payload.tipo)
...
def encerrar_chamado(chamado_id: int, db: Session = Depends(get_db)):
    """
    Executado pelo garçom no celular dele ao chegar na mesa para dar baixa no alerta.
    """
    repo = ChamadoRepository(db)
    if not repo.atender_chamado(chamado_id):
