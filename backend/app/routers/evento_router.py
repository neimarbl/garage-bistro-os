...
def abrir_festa_parametrizada(payload: NovoEventoRequest, db: Session = Depends(get_db)):
    """
...
def vincular_comanda_pvc(payload: VinculoComandaRequest, db: Session = Depends(get_db)):
    """
    Vincula uma comanda física ativa ao evento corporativo do dia.
    """
    repo = EventoRepository(db)
    # ⚡ CORREÇÃO: Chamando obter_evento_ativo (em português) para casar com o repositório
    evento_ativo = repo.obter_evento_ativo()
    if not evento_ativo:
        raise HTTPException(status_code=404, detail="Não há nenhum evento corporativo ativo hoje.")
        
    repo.vincular_comanda_ao_evento(evento_id=evento_ativo.id, comanda_id=payload.comanda_id)
